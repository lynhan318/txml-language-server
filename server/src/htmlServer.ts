...
export function startServer(
    ...
    connection.onInitialize(
        (params: InitializeParams): InitializeResult => {
            ...
            const capabilities: ServerCapabilities = {
                ...
                renameProvider: true,
                //NOTE: current theia not support this option
                ...
                (e: any) => {
                    ...
                    if (workspaceFolders) {
                        for (const folder of workspaceFolders) {
                            if (
                                !toRemove.some(
                                    (r: any) => r.uri === folder.uri
                                ) &&
                                !toAdd.some((r: any) => r.uri === folder.uri)
                            ) {
    ...
    connection.onDidChangeConfiguration((change) => {
        ...
        if (dynamicFormatterRegistration) {
            const enableFormatter =
                globalSettings &&
                globalSettings.txml &&
                globalSettings.txml.format &&
                globalSettings.txml.format.enable;
            if (enableFormatter) {
    ...
    function isValidationEnabled(
        ...
        if (validationSettings) {
            return (
                (languageId === "tcss" &&
                    validationSettings.styles !== false) ||
    ...
    async function validateTextDocument(textDocument: TextDocument) {
        try {
            ...
            if (textDocument.languageId === "txml") {
