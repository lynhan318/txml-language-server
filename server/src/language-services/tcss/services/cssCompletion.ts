...
export class CSSCompletion {
  ...
  private getPropertyProposals(
    ...
    const triggerPropertyValueCompletion = this
      .isTriggerPropertyValueCompletionEnabled;
    const completePropertyWithSemicolon = this
  ...
  protected getColorProposals(
    ...
    for (const p of languageFacts.colorFunctions) {
      let tabStop = 1;
      const replaceFunction = (_match: string, p1: string) =>
        "${" + tabStop++ + ":" + p1 + "}";
  ...
  private makeTermProposal(
    ...
    const insertText =
      symbol.name +
      "(" +
      params.map((p, index) => "${" + (index + 1) + ":" + p + "}").join(", ") +
      ")";
    return {
      label: symbol.name,
  ...
  private doesSupportMarkdown() {
    if (!isDefined(this.supportsMarkdown)) {
      ...
      const documentationFormat = this.lsOptions.clientCapabilities.textDocument
...
function isColorString(s: string) {
  ...
  return (
    s.toLowerCase() in languageFacts.colors ||
