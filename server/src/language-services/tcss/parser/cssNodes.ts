...
export class Node {
  ...
  public endsWith(str: string): boolean {
    return (
      this.length >= str.length && this.getTextProvider()(this.end - str.length, str.length) === str
    );
  }
  ...
  public isErroneous(recursive: boolean = false): boolean {
    ...
    return (
      recursive && Array.isArray(this.children) && this.children.some((c) => c.isErroneous(true))
    );
  }
