...
export function startClient(
    ...
    let client = newLanguageClient(
        "txml",
        localize("htmlserver.name", "TXML Language Server"),
        clientOptions
    );
                ...
                const provider: DocumentSemanticTokensProvider &
                    ...
                    async provideDocumentRangeSemanticTokens(doc, range) {
                        const params: SemanticTokenParams = {
                            ...
                            ranges: [
                                client.code2ProtocolConverter.asRange(range),
                            ],
                        };
                        return client
                            .sendRequest(SemanticTokenRequest.type, params)
                ...
                documentSelector,
                {
                    provideDocumentRangeFormattingEdits(
                        ...
                        const fileFormattingOptions = {
                            ...
                            trimFinalNewlines: filesConfig.get<boolean>(
                                "trimFinalNewlines"
                            ),
    ...
    languages.registerCompletionItemProvider(documentSelector, {
        provideCompletionItems(doc, pos) {
            ...
            if (match) {
                ...
                let beginProposal = new CompletionItem(
                    "#region",
        ...
        if (!config.get("linkedEditing") && !config.get("renameOnType")) {
            const activeEditorListener = window.onDidChangeActiveTextEditor(
                async (e) => {
                    if (
                        e &&
                        documentSelector.indexOf(e.document.languageId) !== -1
                    ) {
                        ...
                        const configure = localize(
                            "configureButton",
                            "Configure"
                        );
                        const res = await window.showInformationMessage(
                            localize(
                                "linkedEditingQuestion",
